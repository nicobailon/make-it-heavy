...
def test_agent_factory_falls_back_to_openrouter_for_invalid_provider(
    ...
    """When an invalid provider is specified, factory raises validation error."""
    ...
    # When/Then: Creating agent with invalid provider raises ValueError
    with pytest.raises(ValueError, match="Provider 'nonexistent_provider' configuration not found"):
        agent = create_agent(tmp_config, silent=True, client=mock_openai_client)
